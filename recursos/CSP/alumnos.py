...
# modificacions para encontrar todas las soluciones
print(len(problem.getSolutions()))

# No obstante esta solución no implementa la restricción que obliga que todas las partes existan. 
# Para ello creamos la siguiente función:
def todasPartesAsignadas(*arg):
    return all([x in list(arg) for x in [1,2,3]])
# Aplicamos la función cmoo una restricción en el problema:
problem.addConstraint(todasPartesAsignadas, ['A', 'Y', 'R', 'F', 'J', 'M'])

# Volvemos a imprimir una solución:
print(problem.getSolution())
